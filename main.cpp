...
// shared EventQueue for scheduling client re-registration
events::EventQueue      *_shared_queue = mbed::mbed_event_queue();
...
#if defined(MBED_CLOUD_CLIENT_TRANSPORT_MODE_UDP) || defined(MBED_CLOUD_CLIENT_TRANSPORT_MODE_UDP_QUEUE)
static bool register_update_called = false;
/**
 * shared EventQueue callback handler for client re-registration
 */
static void reregister_callback(void)
{
...
void registered(const ConnectorClientEndpointInfo *endpoint) {
    ...
    /* When using UDP, start re-registration 30s after initial registration. Allows update campaign to start. */
    #if defined(MBED_CLOUD_CLIENT_TRANSPORT_MODE_UDP) || defined(MBED_CLOUD_CLIENT_TRANSPORT_MODE_UDP_QUEUE)
    if(!register_update_called) {
        register_update_called = true;
        _shared_queue->call_in(30000, reregister_callback);
    }
    #endif
}
...
int main(void) {
    ...
    // Initialize client only after making sure FS is ready
    client = new SimpleMbedCloudClient(net, bd, &fs);

    printf("Starting Simple Pelion Device Management Client example\n");
    //Uncomment the following line to demonstrate update
    //printf("FW updated OTA\n");
